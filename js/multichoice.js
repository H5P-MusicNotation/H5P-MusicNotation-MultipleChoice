 ...
 * @returns {H5P.MultiChoiceScore4LMS}
 * @constructor
 */
H5P.MultiChoiceScore4LMS = function (options, contentId, contentData) {
  if (!(this instanceof H5P.MultiChoiceScore4LMS))
    return new H5P.MultiChoiceScore4LMS(options, contentId, contentData);
  var self = this;
  this.contentId = contentId;
  this.contentData = contentData;
  H5P.Question.call(self, 'MultiChoiceScore4LMS');
  var $ = H5P.jQuery;
  ...
  self.registerDomElements = function () {
    ...
    var $answers = $('.h5p-answer', $myDom).each(function (i) {
      ...
      // Add tip
      var $wrap = $('<div/>', {
        'class': 'h5p-MultiChoiceScore4LMS-tipwrap',
        'aria-label': params.UI.tipAvailable + '.'
      });

      var $MultiChoiceScore4LMSTip = $('<div>', {
        'role': 'button',
        'tabindex': 0,
        'title': params.UI.tipsLabel,
        'aria-label': params.UI.tipsLabel,
        'aria-expanded': false,
        'class': 'MultiChoiceScore4LMS-tip',
        appendTo: $wrap
      });
      ...
      $MultiChoiceScore4LMSTip.append(tipIconHtml);

      $MultiChoiceScore4LMSTip.click(function () {
        var $tipContainer = $MultiChoiceScore4LMSTip.parents('.h5p-answer');
        var openFeedback = !$tipContainer.children('.h5p-feedback-dialog').is($feedbackDialog);
        removeFeedbackDialog();

        // Do not open feedback if it was open
        if (openFeedback) {
          $MultiChoiceScore4LMSTip.attr('aria-expanded', true);
          ...
          $MultiChoiceScore4LMSTip.attr('aria-expanded', false);
        }
    ...
    self.addButton('try-again', params.UI.tryAgainButton, function () {
      ...
      if (params.behaviour.randomAnswers) {
       ...
       for (i = 0; i < answersDisplayed.length; i++) {
         tip[i] = $(answersDisplayed[i]).find('.h5p-MultiChoiceScore4LMS-tipwrap');
       }
  ...
  H5P.MultiChoiceScore4LMS.counter = (H5P.MultiChoiceScore4LMS.counter === undefined ? 0 : H5P.MultiChoiceScore4LMS.counter + 1);
  params.role = (params.behaviour.singleAnswer ? 'radiogroup' : 'group');
  params.label = 'h5p-mcq' + H5P.MultiChoiceScore4LMS.counter;
...
H5P.MultiChoiceScore4LMS.prototype = Object.create(H5P.Question.prototype);
H5P.MultiChoiceScore4LMS.prototype.constructor = H5P.MultiChoiceScore4LMS;
